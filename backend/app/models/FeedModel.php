...
class FeedModel
{
    ...
    {
        $request = "select posts.*, count(likes.id) as likesCount, users.* from posts LEFT join likes on posts.id = likes.post_id left join users on posts.UserID = users.id group by posts.id";
        $stmt = $this->db->prepare($request);
        $stmt->execute();
        $feed = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $feed;
